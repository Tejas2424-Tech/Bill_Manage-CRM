...
// 1. Fetch Branch Info (resolved after POST if admin — see below)
$branch_code = 'MAIN';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // For superadmin (null branch_id), derive branch from the first product in cart
    if (!$branch_id && isAdmin()) {
        $tmp = $pdo->prepare("SELECT branch_id FROM products WHERE id = ? LIMIT 1");
        $tmp->execute([$cart[0]['id']]);
        $branch_id = (int)$tmp->fetchColumn();
    }

    // Resolve branch code now that branch_id is known
    ...
    try {
        ...
        $bill_number = generateBillNumber($branch_code);

        // 3. Insert Bill
        $stmt = $pdo->prepare("INSERT INTO bills (branch_id, bill_number, customer_name, customer_phone, collector_name, bill_type, subtotal, discount_amount, discount_percent, gst_amount, gst_percent, total_amount, paid_amount, status, created_by) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
        $stmt->execute([$branch_id, $bill_number, $customer_name, $customer_phone, $collector_name, $bill_type, $subtotal, $discount_amount, $discount_percent, $gst_amount, $gst_percent, $total_amount, $received_amount, $status, $_SESSION['user_id']]);
        $bill_id = $pdo->lastInsertId();

        // 4. Insert Items and Update Stock
        foreach ($cart as $item) {
            // Validate stock — admin can bill from any branch, staff restricted to their branch
            if (isAdmin()) {
                $check = $pdo->prepare("SELECT quantity, name FROM products WHERE id = ?");
            ...
            $stmt = $pdo->prepare("INSERT INTO bill_items (bill_id, product_id, product_name, selling_price, quantity, discount, total) VALUES (?, ?, ?, ?, ?, ?, ?)");
            $stmt->execute([$bill_id, $item['id'], $item['name'], $item['price'], $item['qty'], $item['discount'], $item['total']]);
...
<div class="pos-wrapper">
    ...
    <div class="pos-right">
        <form id="billForm" method="POST">
            ...
            <!-- ── GENERATE BILL ── -->
            <button type="submit" class="btn btn-primary" style="width:100%;height:50px;font-size:15px;font-weight:700;letter-spacing:0.3px;border-radius:var(--radius);">
...
function updateQty(idx, val) {
    ...
    cart[idx].qty = val;
    cart[idx].total = Math.max(0, (cart[idx].price * val) - cart[idx].discount);
    renderCart();
    recalculate();
}
...
function updateDiscount(idx, val) {
    ...
    const maxDisc = cart[idx].price * cart[idx].qty;
    if (val > maxDisc) { val = maxDisc; }
    cart[idx].discount = val;
    cart[idx].total = Math.max(0, (cart[idx].price * cart[idx].qty) - val);
    renderCart();
    recalculate();
}
...
function renderCart() {
    ...
    tb.innerHTML = cart.map((item, i) => `
        <tr>
            <td>
                <div style="font-weight:600;color:var(--on-surface);font-size:13px;">${item.name}</div>
                ${item.showDisc ? `
                <div class="disc-row">
                    <span style="font-size:11px;color:var(--on-surface-subtle);">Item disc (₹):</span>
                    <input class="disc-input" type="number" value="${item.discount.toFixed(2)}"
                           min="0" max="${(item.price * item.qty).toFixed(2)}" step="0.01"
...
function recalculate() {
    ...
    const grandTotal = subtotal - billDisc + gst;
...
smartSearch.addEventListener('keydown', e => {
    ...
    if (/^\d{8,}$/.test(val)) {
        // Barcode scan
        fetch('search_product.php?barcode=' + encodeURIComponent(val))
            .then(r => r.json())
            .then(p => {
...
function doSearch(q, addFirst) {
    fetch('search_product.php?q=' + encodeURIComponent(q))
        .then(r => r.json())
        .then(data => {
